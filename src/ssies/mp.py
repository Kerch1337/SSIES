...
from ssies.schemes import HEADER_SCHEMA_2, MP_SCHEMA
import xarray as xr
import numpy as np
import pandas as pd
...
class MP(SSIES):
    ...
    def parse_file(self):
        ...
        with self._open_file() as file:
            while True:
                try:
                    minute_record = self._parse_minute_record(file, HEADER_SCHEMA_2, MP_SCHEMA, "number_set")
    ...
    def _to_xarray(self, records):
        ...
        set_names = [f["name"] for f in MP_SCHEMA]
        ...
        self._apply_schema_attrs(ds, MP_SCHEMA)
